...
import { forceLogout, setupAuthInterceptors } from "@/services/auth-interceptor"
...
function App() {
  ...
  useEffect(() => {
    ...
    const onSessionExpired = (payload: AuthEventPayload) => {
      let expiredMessage = t("auth:errors.session_expired", {
        defaultValue: "Phiên đăng nhập đã hết hạn, vui lòng đăng nhập lại"
      })

      if (payload.reason === "revoked") {
        expiredMessage = t("auth:errors.session_revoked", {
          defaultValue: "Tài khoản của bạn đã bị vô hiệu hóa hoặc đăng nhập ở nơi khác"
        })
      }
    ...
    // Unsubscribe from global listeners when root App component unmounts
    return () => {
